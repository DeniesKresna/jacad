...
class PlaylistController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $datas = Playlist::query()->where('customer_id',$session_id)->orderBy("id","desc")->with(['content_masters' => function($q){
            $q->orderBy('order_no');
    ...
    {
        $session_id = request()->get('auth')->user->id;
        $data = Playlist::with(['content_masters' => function($q){
            $q->orderBy('order_no');
        }])->findOrFail($id);
    ...
    public function store(Request $request){
        ...
        if($playlist){
            if($request->file_ids){
                ...
                $playlist->content_masters()->sync($sync_data);
            }
        }
        else
            return self::error_responses("Unknown error");

        return self::success_responses($playlist->load(['content_masters' => function($q){
            $q->orderBy('order_no');
        }]));
    }
