...
class CampaignController extends ApiController
{
    ...
    public function show($id)
    {
        ...
        $data = Campaign::withTrashed()->with("customer","verificator","locations","content_masters","device_types")->findOrFail($id);
        ...
        if($res){
            if($res->customer_id == $session_id){
                if($res->status != 1){
                    ...
                    $media_paths = $res->content_masters()->pluck('file_path');
                    foreach ($media_paths as $media_path) {
                        @unlink(base_upload_dir().$media_path);
                    }
                    $res->content_masters()->delete();
