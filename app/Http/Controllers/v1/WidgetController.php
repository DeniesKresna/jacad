...
class WidgetController extends ApiController
{
    public function forecasts(Request $request){
        ...
        if($request->mode=='half' || $request->mode=='halfapi'){
            ...
            else if($forecast['condition_text'] == 'Partly Cloudy') $forecast['condition_text'] = 'Part cloud';
    ...
    public function prayhours(Request $request){
        ...
        if($request->has('mode')){
            ...
            else if($request->mode=='runninggeo'){
                $dataSalat['data']['timings']['street_name'] = $this->geo($latitude,$longitude);
                return view('widget.prayhourrunninggeo',$dataSalat['data']['timings']);
            }
            else
