...
class DeviceController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $datas = $datas->orderBy('id', 'desc')->with(['creator', 'location', 'device_line', 'device_line.box', 'device_line.car', 'device_line.driver', 'device_line.merchant', 'device_line.layout', 'device_line.layout.boxes', 'device_type'])->paginate($page_size, ["devices.*"], "page", $page);
    ...
    public function list(Request $request)
    {
        ...
        if ($request->has('active')) $datas = $datas->where('active', filter_var($request->active, FILTER_VALIDATE_BOOLEAN));
        if ($request->has('device_line_except') && $request->device_line_except) {
            $datas = $datas->doesntHave('device_line');
        }
        if ($request->search) $datas = $datas->where('imei', 'like', '%' . $search . '%');
        $datas = $datas->orderBy('id', 'desc')->get();
        $datas = to_array($datas);
        if ($request->has('device_line_owner') && $request->device_line_owner) {
            $device_line = DeviceLine::query()->where('id', '=', $request->device_line_owner)->first();
            if ($device_line) {
                $device = $device_line->device;
                if ($device) {
                    if ($datas) {
                        array_push($datas, to_array($device));
                    } else {
                        $datas = [];
                        array_push($datas, to_array($device));
                    }
    ...
    public function store(Request $request)
    {
        ...
        if ($result)
            return self::success_responses($result->load(['creator', 'location', 'device_line', 'device_line.box', 'device_line.car', 'device_line.driver', 'device_line.merchant', 'device_line.layout', 'device_line.layout.boxes', 'device_type']));
        else
    ...
    public function show($id)
    {
        ...
        if ($res)
            return self::success_responses($res->load(['creator', 'location', 'device_line', 'device_line.box', 'device_line.car', 'device_line.driver', 'device_line.merchant', 'device_line.layout', 'device_line.layout.boxes', 'device_type']));
        else
    ...
    {
        $datas = DB::table('devices')->where('last_gps_time', '>', DB::raw('NOW() - INTERVAL 1 Day'))->orderBy('last_gps_time', 'desc')
            ->select("imei", "last_gps_time as last_on_time")
            ->get();
