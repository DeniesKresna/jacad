...
class ContentMasterController extends ApiController
{
    ...
    public function list(){
        ...
        $datas = ContentMaster::query()->where('id', '>', 0);
        if ($request->search) $datas = $datas->where('name', 'like', '%' . $search . '%');
        $datas = $datas->orderBy('name', 'asc')->get();
    ...
    public function destroy($id)
    {
        ...
        foreach ($contents as $content) @unlink(upload_dir() . $content->file_path);
        $contents->delete();
        $res->delete();
        if ($res)
            return self::success_responses("Success delete content");
        else
            return self::error_responses("Delete failed");
    }
}
